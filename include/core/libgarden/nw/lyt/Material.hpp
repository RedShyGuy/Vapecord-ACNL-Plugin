...
namespace nw::lyt
{
	...
	public:
		...
		virtual ~Material() {
			...
			static const FN_Destructor destructor = reinterpret_cast<FN_Destructor>(Address(0x4BE8A8).addr);
			destructor(this);
		}
		...
		void ReserveMem(u8 texMapCount, u8 texMatrixCount, u8 texCoordGenCount,
			u8 tevStageCount = 0, bool alphaCompare = false, bool blendMode = false) {
				...
				static const FN_ReserveMem reserveMem = reinterpret_cast<FN_ReserveMem>(Address(0x4BC900).addr);
