...
namespace CTRPluginFramework {
	...
	ExHandler Wrap::Delete(const std::string& path, const std::string& filetype) { 
		...
		if((MessageBox(Utils::Format(Language->Get("FILE_PROMPT").c_str(), f_list.at(uchoice).c_str()), DialogType::DialogYesNo)).SetClear(ClearScreen::Top)()) {
			...
			MessageBox(Utils::Format(Language->Get("FILE_RES1").c_str(), f_list.at(uchoice).c_str())).SetClear(ClearScreen::Top)();
		}
		dir.Close();
		return ExHandler::SUCCESS; //success
	}
