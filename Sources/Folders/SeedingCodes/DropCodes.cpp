...
//Drop Modifier /*Radius calculations done by Nico*/
	void dropMod(MenuEntry *entry) {		
		...
		if(entry->WasJustActivated()) {
			Process::Patch(Address(0x19CFFC).addr, 0xE1A00000);
			Process::Write8(Address(0x660430).addr, 6); //set anim after bury
		}
	...
	//Modify Drop Type	
		if(entry->Hotkeys[0].IsPressed()) { 
			...
			Process::Write8(Address(0x59771C).addr, DropType);
			return;
		}
	...
	//Modify Drop Radius
		if(entry->Hotkeys[1].IsPressed()) {
			...
			//Full Square
				case 0: {
					if(Wrap::KB<u8>(Language::getInstance()->get("DROP_MODS_RADIUS") << "\nMax Value: 0x15", true, 1, val, 0)) {
						...
						range = val * 2 + 1;
						count = range * range;
						Process::Write8(Address(0x599348).addr, count);
						Process::Write8(Address(0x59915C).addr, count);
						for(s8 i = 0 - (s8)val; i <= (s8)val; ++i) {
							for(u8 j = 0; j < range; ++j) {
								Process::Write8(Address(0x85FE58).addr + j + ((i + val) * range), (0 - (s8)val) + j);
								Process::Write8(Address(0x85FE58).addr + count + j + ((i + val) * range), i);
							}
						}	
					} 
					DropPatternON = true;
				...
				case 1: {
					if(Wrap::KB<u8>(Language::getInstance()->get("DROP_MODS_RADIUS") << "\nMax Value: 9", true, 1, val, 0)) {
						...
						for(u8 i = 0; i < count; ++i) {
							Process::Write8(Address(0x85FE58).addr + i, (s8)nearbyint((float)(val * cos((6.28318530 / count) * i))));
							Process::Write8(Address(0x85FE58).addr + count + i, (s8)nearbyint((float)(val * sin((6.28318530 / count) * i))));
						}
					}
					DropPatternON = true;
				} break;
			//horizontal line	
				case 2: {
					if(Wrap::KB<u8>(Language::getInstance()->get("DROP_MODS_RADIUS") << "\nMax Value: 0x10", true, 2, val, 0)) {
						...
						range = val * 2 + 1;
						Process::Write8(Address(0x599348).addr, range);
						Process::Write8(Address(0x59915C).addr, range);
						for(s8 i = 0 - (s8)val; i <= (s8)val; ++i) {
							Process::Write8(Address(0x85FE58).addr + (i + val), i);
							Process::Write8(Address(0x85FE58).addr + range + (i + val), 0);
						}
			...
			//Vertical line	
				case 3: {
					if(Wrap::KB<u8>(Language::getInstance()->get("DROP_MODS_RADIUS") << "\nMax Value: 0x10", true, 2, val, 0)) {
						...
						Process::Write8(Address(0x59915C).addr, range);
						for(s8 i = 0 - (s8)val; i <= (s8)val; ++i) {
							Process::Write8(Address(0x85FE58).addr + (i + val), 0);
							Process::Write8(Address(0x85FE58).addr + range + (i + val), i);
						}
				...
				case 4: {
					if(Wrap::KB<u8>(Language::getInstance()->get("DROP_MODS_RADIUS") << "\nMax Value: 0x10", true, 2, val, 0)) {
						...
						for(u8 i = 0; i < range; ++i) {
							Process::Write8(Address(0x85FE58).addr + i * 4, i - val);
							Process::Write8(Address(0x85FE58).addr + count + i * 4, 0 - val);
							Process::Write8(Address(0x85FE58).addr + 1 + i * 4, 0 - val);
							Process::Write8(Address(0x85FE58).addr + 1 + count + i * 4, i - val);
							Process::Write8(Address(0x85FE58).addr + 2 + i * 4, i - val);
							Process::Write8(Address(0x85FE58).addr + 2 + count + i * 4, val);
							Process::Write8(Address(0x85FE58).addr + 3 + i * 4, val);
							Process::Write8(Address(0x85FE58).addr + 3 + count + i * 4, i - val);
						}
					}
					DropPatternON = true;
				} break;
			...
			//reset drop pattern	
				case 7: {
					Process::Write8(Address(0x599348).addr, 9);
					Process::Write8(Address(0x59915C).addr, 9);
					Process::Patch(Address(0x599248).addr, 0x0A000006);
					Process::Patch(Address(0x5991E8).addr, 0xE7971100);
			...
			Process::Patch(Address(0x599248).addr, 0xEA000006);
			Process::Patch(Address(0x5991E8).addr, 0xE1A01000);
			return;
		}
	...
	//set Item sequence	
		...
		if(!entry->IsActivated()) {
			Process::Patch(Address(0x30B83C).addr, 0x13A00001);
			Process::Patch(Address(0x19CFFC).addr, 0x0A000003);
			Process::Patch(Address(0x660430).addr, 0xE3A01016);
			Process::Patch(Address(0x59771C).addr, 0xE3A0000C);
			Process::Patch(Address(0x599348).addr, 0xE3540009);
			Process::Patch(Address(0x59915C).addr, 0xE28AB009);
			Process::Patch(Address(0x599248).addr, 0x0A000006);
			Process::Patch(Address(0x5991E8).addr, 0xE7971100);
