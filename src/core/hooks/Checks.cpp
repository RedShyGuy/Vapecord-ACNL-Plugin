...
namespace CTRPluginFramework {
	void CheckInvalidBadge(u32 data, u32 badge, int badgeType, u32 r3, u32 r4) {
		if (badgeType > 3) {
			HUD::Notify(Language::getInstance()->get(TextID::CHECKS_INVALID_BADGE), Color::Red);
			return;
		}
...
//Hook for invalid item check
	bool InvalidItemStop(Item* item) {
		if(!item->isValid(false)) {
			...
			if(!IDChecks::IsInRange(item->ID, 0x30CC, 0x30D1)) {
				return false;
			}
		}
		return true;
	...
	bool ConvertFlower/*And Mushroomized Furniture*/(Item *item) {
		if(IDChecks::IsInRange(item->ID, 0x9F, 0xCB)) {
			Game::ToIndoorFlowers(*item);
		}
		else if(IDChecks::IsInRange(item->ID, 0x2120, 0x212A)) {
			item->ID += 0x959;
		}
...
//Hook to check if item is replacable
	...
	bool IsItemDroppable(u32 ItemData, Item *ItemID, int SecondaryItemFlag) {
		if(IDChecks::IsInRange(ItemID->ID, 0x98, 0x9E)) {
			return true;
		}
		else if(ItemID->ID == 0xFD) {
			return true;
		}
		else if(IDChecks::IsInRange(ItemID->ID, 0x228E, 0x234B)) {
			return true;
		}
		else if(IDChecks::IsInRange(ItemID->ID, 0x340C, 0x34CD)) {
			return true;
		}
	...
	//seed items which should be plantable
		if(IDChecks::IsInRange(ItemID->ID, 0, 0x97)) {
			return true;
		}
		else if(IDChecks::IsInRange(ItemID->ID, 0x9F, 0xFC)) {
			return true;
		}
	...
	int CatalogPatch_Keyboard(u32 u0, u32 u1, u32 u2) {
		if(!Game::IsGameInRoom(0x38) && !Game::IsGameInRoom(0x39) && !Game::IsGameInRoom(0x3A) && !Game::IsGameInRoom(0x3B) && !Game::IsGameInRoom(0x3C)) {
			HUD::Notify(Language::getInstance()->get(TextID::CHECKS_SEARCH_NOT_SUPPORTED), Color::Red);
			return 0;
		}
...
//basically "forces" a B press directly for the search function to break
	...
	*/
	...
	bool FruitStacking(u32 InvAddress, int slot, Item *item, u32 ItemLock, u32 ItemToReplace) {
		...
		bool isFruit = IDChecks::IsInRange(item->ID, fruitMin, fruitMax);
		bool isBasket = IDChecks::IsInRange(item->ID, fruitMin + basketOffset, fruitMax + basketOffset);
